...
- macOS install path: a Homebrew cask in `shawonashraf/homebrew-tap`,
  regenerated by a workflow when a release is published. Its postflight runs
  `xattr -cr` on the installed app so users skip the manual quarantine step.
